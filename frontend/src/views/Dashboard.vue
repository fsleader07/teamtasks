<template>
  <div class="space-y-6 relative min-h-screen p-4 bg-gray-50">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div
        v-for="(stat, key) in statConfig"
        :key="key"
        class="bg-white rounded shadow-sm flex border border-gray-200 overflow-hidden"
      >
        <div :class="['w-1/3 flex items-center justify-center text-white text-2xl', stat.color]">
          {{ stat.icon }}
        </div>
        <div class="p-3">
          <p class="text-[10px] text-gray-500 uppercase font-bold">{{ stat.label }}</p>
          <p class="text-xl font-bold text-gray-800">{{ statusCounts[key] }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div class="relative">
              ...
              <input
                ...
                class="w-full bg-white border border-gray-300 text-gray-800 text-sm rounded py-2 pl-9 pr-4 focus:ring-1 focus:ring-blue-500 outline-none"
              />
            </div>
            <select
            ...
            <div class="flex items-center gap-3">
              <select
                ...
                class="flex-1 bg-white border border-gray-300 text-sm rounded py-2 px-3 outline-none focus:ring-1 focus:ring-blue-500"
              >
                <option value="">ผู้รับผิดชอบทั้งหมด</option>
                <option
                  v-for="user in allPersonnel"
                  :key="user.person_id"
                  :value="user.nickname || user.firstname"
                >
            ...
            <table class="w-full text-left text-sm">
              <thead class="bg-gray-50 text-gray-600 font-bold uppercase text-[11px] border-b">
                <tr>
                  ...
                  <th class="px-4 py-3 border-r cursor-pointer" @click="sortBy('project')">
                    Project
                  </th>
                  <th class="px-4 py-3 border-r cursor-pointer" @click="sortBy('task_name')">
                    Task
                  </th>
              ...
              <tbody class="divide-y divide-gray-100">
                ...
                >
                  <td class="px-4 py-3 text-center text-gray-500 text-xs border-r">
                    {{ index + 1 }}
                  </td>
                  <td class="px-4 py-3 border-r font-bold text-[#007bff] text-[10px]">
                    {{ task.project || '-' }}
                  </td>
                  <td class="px-4 py-3 border-r">
                    <div class="font-semibold text-gray-800">{{ task.task_name }}</div>
                    <div v-if="task.deadline" class="text-[9px] text-gray-400">
      ...
      <div class="lg:col-span-4 space-y-4">
        ...
        <div
          class="bg-white rounded shadow-sm border border-gray-200 p-2 overflow-hidden sticky top-4"
        >
      ...
      >
        ...
        <div class="flex gap-2 justify-end">
          <button
            ...
            class="px-4 py-2 bg-gray-200 text-gray-700 rounded text-sm font-medium"
          >
            ยกเลิก
          </button>
          <button
            @click="confirmDelete"
            class="px-4 py-2 bg-red-600 text-white rounded text-sm font-bold shadow-md"
          >
            ลบรายการ
          </button>
        </div>
...
const loadData = async () => {
  try {
    const [tasks, counts, projects, personnel] = await Promise.all([
      TaskService.fetchMyTasks('all'),
      TaskService.fetchStatusCounts(),
...
const formatDate = (d) =>
  d ? new Date(d).toLocaleDateString('th-TH', { day: '2-digit', month: '2-digit' }) : '-'
