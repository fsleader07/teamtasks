<template>
  <div class="space-y-6 relative min-h-screen p-4">
    ...
    <div class="text-right">
      <button @click="openAddModal" class="mt-2 md:mt-0 bg-[#007bff] hover:bg-[#0069d9] text-white px-4 py-2 rounded text-sm font-bold shadow transition active:scale-95">
        + เพิ่มงานใหม่
      </button>
    </div>
    ...
    <div class="bg-white rounded shadow-sm p-4 border border-gray-200">
      <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
        ...
        <select v-model="filters.project" class="bg-white border border-gray-300 text-sm rounded py-2 px-3 outline-none focus:ring-1 focus:ring-blue-500">
          ...
          <option v-for="proj in allProjects" :key="proj.project_name" :value="proj.project_name">
            {{ proj.project_name }}
          </option>
        </select>
        ...
        <select v-model="filters.assignee" class="bg-white border border-gray-300 text-sm rounded py-2 px-3 outline-none focus:ring-1 focus:ring-blue-500">
          ...
          <option v-for="user in allPersonnel" :key="user.person_id" :value="user.nickname || user.firstname">
            {{ user.nickname || user.firstname }}
          </option>
        </select>

        <div class="flex bg-gray-100 p-1 rounded-lg">
          <button @click="switchView('table')" :class="['px-3 py-1.5 rounded-md text-xs font-bold transition-all w-1/2', currentView === 'table' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700']">
            📋 ตาราง
          </button>
          <button @click="switchView('calendar')" :class="['px-3 py-1.5 rounded-md text-xs font-bold transition-all w-1/2', currentView === 'calendar' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700']">
            📅 ปฏิทิน
          </button>
        </div>

        <button @click="resetFilters" class="text-blue-600 hover:text-blue-800 text-sm font-medium flex items-center justify-center h-full">
          🔄 ล้างค่า
        </button>
      </div>
    </div>

    <div v-show="currentView === 'table'" class="bg-white rounded shadow-sm card-primary card-outline overflow-hidden border border-gray-200">
    ...
    <div v-show="currentView === 'calendar'" class="bg-white rounded shadow-sm card-primary card-outline overflow-hidden border border-gray-200">
      <div class="px-4 py-3 border-b border-gray-100 font-bold text-gray-700 text-sm">
        ตารางงาน (ปฏิทิน)
      </div>
      <div class="p-4">
...
const switchView = (view) => {
  ...
  if (view === 'calendar') {
    // TODO: เรียกใช้งาน Function Render Calendar ตรงนี้
    // เช่น CalendarService.render(filteredTasks.value)
  }
};
...
const loadData = async () => {
  try {
    ...
    // หากอยู่ในหน้า Calendar ให้ Update ข้อมูลลงปฏิทินด้วย
...
// ดักจับการเปลี่ยนแปลงของ Filter เพื่อส่งข้อมูลให้ Calendar อัปเดตแบบ Real-time
watch(filteredTasks, (newTasks) => {
  if (currentView.value === 'calendar') {
    // updateCalendarEvents(newTasks);
  }
});
...
const handleCalendarEventClick = (taskId) => {
  // ค้นหา Object ของงานเต็มๆ จาก ID เพื่อส่งให้ Modal แก้ไข
  const taskToEdit = allTasks.value.find(t => String(t.id) === String(taskId));
  if (taskToEdit) {
    handleEdit(taskToEdit); // ฟังก์ชันเดิมที่ใช้เปิด Modal
  }
};
</script>